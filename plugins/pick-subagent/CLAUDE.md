# pick-subagent

서브에이전트를 잘 활용하기 위한 도구 모음. 모델을 지정해 가볍게 위임하는 슬래시 커맨드 4개와, 플랜→검증계획→실행→검증을 반복해 품질을 보장하는 subagent-loop 스킬을 함께 담는다.

## 왜 존재하는지

작업을 서브에이전트에 위임할 때 모델을 명시적으로 지정하고 싶을 때 쓴다. `/sub-opus 코드 리뷰해줘`처럼 한 줄로 위임 가능.
subagent-loop 스킬은 플랜→검증계획→실행→검증의 파이프라인을 반복하여 높은 품질을 보장한다.

## 언제 쓰면 안 되는지

- 서브에이전트 없이 직접 처리해도 되는 간단한 작업
- 대화 컨텍스트가 중요한 작업 (서브에이전트는 현재 대화를 모름)
- subagent-loop: 토큰 비용이 높으므로 간단한 작업에는 부적합

## 구성

- `commands/sub-opus.md` - `/sub-opus` Opus 모델 서브에이전트
- `commands/sub-sonnet.md` - `/sub-sonnet` Sonnet 모델 서브에이전트
- `commands/sub-haiku.md` - `/sub-haiku` Haiku 모델 서브에이전트
- `commands/sub-fable.md` - `/sub-fable` Fable 모델 서브에이전트
- `skills/subagent-loop/SKILL.md` - 서브에이전트 파이프라인 + 10점 검증 루프
