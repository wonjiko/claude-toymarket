...
- 공용 동작은 한 곳에 둔다.
- Claude/Codex 전용 파일은 얇은 adapter 또는 generated file로 둔다.
...
현재 저장소는 Claude target만 존재한다. Codex target은 `catalog/toymarket.json`을 기준으로 생성 가능한 형태로 설계한다.
...
- `plugins/<plugin>/commands/*.md`: Claude slash command adapter
- `plugins/<plugin>/agents/*.md`: Claude agent definition
...
4. 실제 Claude generated file과 byte-for-byte로 비교한다.
...
3. `python3 scripts/verify_repo.py --profile claude`로 현재 Claude 구조를 검사한다.
